...
- **NPC 배포 도구 행:** 여성 / 남성 / 배포 하기 / 배포 취소. 진입 시 여성이 기본 선택됩니다. 가까운 해당 성별 NPC를 선택하고 없으면 플레이어로 돌아갑니다. 후타는 등록된 여성 후타 NPC만 자동 선택합니다. 배포 목록·새 규칙의 성별은 미리보기 액터와 분리하고, 바디·스킨에는 설정된 배포 바디 타입도 반영합니다. 성별 전환 시 이전 체크·미리보기를 정리합니다.
...
Release build and all **24 automated test executables passed**, including **2,936 morph-key checks**. Both local MO2 installations received the same DLL. This build's in-game verification is pending.
