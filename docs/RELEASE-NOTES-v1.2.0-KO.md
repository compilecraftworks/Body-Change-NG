...
**v1.1.4 대비 최종 구현**을 2026-09-13 기준으로 정리했다. 갱신된 v1.2.0 릴리즈에 대응하며 개발 중간의 실험 기록보다 이 문서를 우선한다. 현재 파일 구성과 알려진 제한은 [GitHub 릴리즈 요약](GITHUB-RELEASE-v1.2.0.md)을 참고한다.
...
- 바디·스킨·후타 배포 목록은 마지막 호환 행을 미리보기한다. 오버레이는 체크한 항목을 누적 표시하며 체크를 풀면 제거한다. 미리보기는 액터 수동 선택으로 저장하지 않는다.
- 후타 등록·애드온 변경 뒤 후타 후보만 다시 평가하며 수동 선택은 보호한다. 전체 NPC용 커스텀 팔로워·노인 제외 체크는 이름·개별 NPC 등 다른 대상 조건에 적용하지 않는다.
- XML 모프는 체중 보간 후 음수·100% 초과 값을 유지한다. 다른 모드의 모프 보존은 기본 켬, 캐릭터 위치는 왼쪽, 게임 일시정지·성능 모드는 기본 해제다.
...
- 호환 바디·의상은 Zeroed Sliders와 Build Morphs로 빌드한다. 게임에 이미 노출된 XML/BodySkin/Futanari 폴더의 파일 추가·교체 후 새로고침할 수 있지만, MO2 새 모드·플러그인·애드온은 재실행이 필요하다.
...
2026-09-13 추가 수정 후 SE/AE Release 빌드와 **테스트 실행 파일 24개**를 통과했다. SFS consumer 경계, NPC·후타 직렬화, 미리보기 복원, 모프 소유권과 UI 좌표 검사를 포함한다. 인게임 검증은 별도다.

오버레이 미리보기 중 저장하면 임시 RaceMenu 노드가 남을 수 있는 경로는 조사 중이다. 저장 전 BCNG를 닫고 미리보기가 복원되도록 한다. [NPC·저장 점검 기록](NPC-DISTRIBUTION-SAVE-AUDIT-20260913-KO.md), [SFS 연동 점검 기록](SFS-OREFIT-INTEGRATION-20260913-KO.md).
