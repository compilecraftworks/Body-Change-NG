...
**Changing one NPC does not require distribution rules.** Select that NPC in the actor dropdown, open Body Skins, single-click a compatible pack to preview, and double-click to apply. NPC Distribution is a separate, optional workflow for assigning your selected candidates by rules.
...
1. Install the matching requirements and the bodies/assets you want to use.
1. Install BCNG with your mod manager. Its DLL must resolve to **Data\SKSE\Plugins\BodyChangeNG.dll**. Keep only one active BCNG DLL.
1. For a normal skin pack, copy its entire installed mod folder into BodySkin, as shown below. MO2's mod root corresponds to the game's Data folder: inside an MO2 mod, start with **BodySkin**, **Futanari**, or **CalienteTools**—do not add another Data folder.
1. Launch through SKSE, using the same mod-manager profile.
1. Press **F7**. The window opens with the player selected. Use **Refresh actors** and the actor dropdown to choose a loaded NPC.
1. Choose a tab, single-click to preview, and **double-click to confirm**. Close without confirming to restore the previously committed selection.
...
### Choosing nearby or distant NPCs

The normal actor dropdown lists nearby, living, enabled NPCs with loaded 3D and a usable name, within 4,096 game units and up to the nearest 32 NPCs. Hostile and non-hostile NPCs—including bandits—are allowed. An eligible actor under the crosshair may also be included when opening the UI. This local list is not the scope of automatic distribution.

Open the actor dropdown, enter a **name or hexadecimal RefID** in its search field, and press **Enter** to search beyond the nearby distance/32-NPC limit. The number beside an actor's name is its reference ID, not the NPC BaseID. Use the individual actor's RefID for a specific actor; names can match multiple references.

Search finds actor references that already exist in the current game. It does not spawn NPCs, load cells, or guarantee that every NPC base in an installed plugin already has a searchable reference. Use distribution rules for NPCs that have not spawned yet. Name searches still exclude dead, disabled, unnamed, and non-NPC entries.

For an unloaded actor, compatible **body, skin, futanari-skin, and overlay choices** can be confirmed and stored without a live preview. When that actor's 3D loads, BCNG rechecks compatibility and processes the saved choice. **Save the game** to retain pending choices across sessions. Eligibility requirements still apply; Tint Masks remains player-only.
...
1. Check **Build Morphs**. Build the character body and its matching parts, then use **Batch Build** for the compatible outfits you will wear, keeping the same zeroed base and Build Morphs enabled.
1. Enable the generated mesh and TRI output in that profile. Ensure another mod does not overwrite it with a body or outfit built to a different shape.
...
Select a BodySlide shape for the player or an NPC. Live preview requires loaded 3D; the actor-search workflow above also supports pending choices for unloaded actors. The list uses the selected actor's sex and detected body family. XML Preset/set and Group metadata help distinguish CBBE 3BA, BHUNP/UNP, UBE, HIMBO, and SAM; renaming an XML does not convert its sliders.

Actor detection prioritizes the live skin geometry and texture layout over stale form or folder labels. Only race-compatible body, hand, and foot addons contribute addon metadata; installing UBE alongside a conventional body does not override a confirmed conventional layout. A custom NPC is not excluded simply because it comes from another mod. This does not convert incompatible meshes or sliders.

The first **Default Body** row clears the selected BCNG preset and old BCNG keys, plus the exact OBody and OClothe keys. Enabled outfit correction is an independent layer: it can still follow clothing on a Default body, including after loading a save or closing RaceMenu. Use the correction options to disable it; explicit actor reset and mod-removal cleanup clear their owned layers. Default Body leaves all other morph keys alone regardless of the preservation option. It does not replace installed meshes or turn a custom body into a vanilla mesh. Your body and outfit must contain the corresponding morph data to show a change.

Saved BCNG morph layers are checked when actors load or their 3D updates. Missing layers are restored from the saved selection without rerolling distribution or interrupting an active preview. This does not stop another body controller from writing its own morphs.
...
- If a body/hand/foot addon keeps its skin textures only in the NIF, BCNG reads the actual skin-material baseline and creates private native targets. Custom paths and normal-map conventions are retained; distinct atlases stay separate. Face-only packs leave the body unchanged and do not prevent later body-skin selection.
...
- The first **Default Skin** row restores the underlying skin instead of another BCNG pack, such as the vanilla skin or an installed texture replacer. This does not prevent another mod from reapplying its own skin changes; see the incompatible mods below.

When the actor's layout is known, the catalog separates UBE from conventional skins; sex and applicable race filters remain in effect. An unknown family is handled by the manual-choice workflow above, not silently reclassified from BCNG's cached filenames. A conventional listing is **not a UV conversion**: choose CBBE or UNP textures for the mesh you actually use. Likewise, a male SOS texture pack must fit your HIMBO/SOS mesh and addon.
...
**SOS/TNG size or addon changes:** BCNG changes supported textures, not the addon's size or mesh. Set size in SOS/TNG. Addon reselection can rebuild 3D, so the saved compatible skin is restored through the normal update path. Default restoration also restores BCNG's retained private texture paths that a provider may still reference. This does not convert ERF/TRX/UBE textures or make different addons interchangeable. If a texture breaks, check the addon and source pack, keep a backup save, and do not delete the cache as a repair attempt.
...
For example, copy the installed **My Skin Pack** folder from MO2's mods directory into **Body Change NG\BodySkin**. Keep its existing Textures directory and all its contents.

```text
Before — relative to your MO2 installation:
mods\My Skin Pack\Textures\actors\character\female\femalehead.dds

After — inside the BCNG mod:
mods\Body Change NG\BodySkin\My Skin Pack\Textures\actors\character\female\femalehead.dds
...
- **BodyChangeNGdistribution.json:** explicitly saved NPC rules. There are no Import/Export buttons. Back up or copy this file with the game closed; a valid file at the full path above is loaded automatically on the next game launch. Eligible NPCs are processed as they load, unless removal mode is active; manual choices still take priority. Referenced packs/plugins must remain available. Keep your customized file when upgrading; the installer starter is empty.

- **Runtime texture cache:** generated DDS copies under Data\Textures\BodyChangeNG\Cache\. This is output, not your source-pack folder. Keep it available to the same mod-manager profile. Older saves can still reference these paths even when the current actor uses another skin; do not bulk-delete the cache. Use the optional offline maintenance tool below to reclaim eligible duplicate storage without removing DDS paths.
- **OBody ORefit JSON:** use [ORefit JSON Master List](https://www.nexusmods.com/skyrimspecialedition/mods/105052) for outfit-correction rules, then click **Register ORefit outfit-correction rules** in BCNG.
...
### Preview restoration and large catalogs

Unconfirmed previews remain temporary. Cancellation is retained if an NPC unloads; player body/skin/overlay undo waits while RaceMenu is editing the player and resumes after it closes. A newer selection takes priority over older pending work. Default Skin completes only after both the owned body state and face cleanup finish.

Preset, skin, futanari, and tint lists reuse catalog data and avoid drawing off-screen rows while retaining keyboard navigation and distribution checkboxes. Mouse buttons and wheel input follow Skyrim's input stream and game cursor. These changes reduce repeated list work and missed-input paths; they do not make texture loading instantaneous.

### Prepare for mod removal

1. Back up your save and its matching .skse co-save while BCNG is still installed.
1. Open **Mod Settings → Prepare for removal... → Start cleanup**. This resets BCNG selections and restores its managed appearance, and persistently suspends automatic distribution and corrections.
1. Wait for **Cleanup verified**. Do not save or uninstall while cleanup is in progress. If it reports incomplete, leave BCNG installed, let pending updates finish or load the remaining actors, then retry.
1. After verification, close BCNG, save to a **new slot**, fully exit Skyrim, and uninstall.

Your rule file and other mods' morph keys are kept. Removal mode persists in **Data\SKSE\Plugins\BodyChangeNG\settings.json**. If you decide to keep the mod, **Resume BCNG** exits that mode; it does not recover selections already erased by cleanup. An older save has its own state and must be cleaned separately before removal. This is not a general damaged-save repair tool and does not make old cache paths disposable.

### Safe cache maintenance — optional, outside the game

The release includes **Tools\BodyChangeNGCache.exe** and **Tools\BodyChangeNGCache-README.txt**. After installation these correspond to **Data\Tools\BodyChangeNGCache.exe** and **Data\Tools\BodyChangeNGCache-README.txt**; in MO2, open the installed mod's physical Tools folder.

1. Close Skyrim and texture editors.
1. Run **BodyChangeNGCache.exe directly**, not through MO2.
1. Select the physical MO2 Overwrite/output folder, or its **textures\BodyChangeNG\Cache** directory.
1. Review the read-only scan. Choose **Yes** to compact eligible duplicates; **No** is the default.

Only byte-identical DDS copies with matching modification times are combined into NTFS hard links. Every DDS path and its content are preserved, including paths used by older saves. Unique textures are not deleted just because they are old or not currently selected. Only recognized unpublished preparation/link temporary files are removed.

The tool skips external hard links, read-only/locked files, junctions and unknown layouts. It does not edit source packs, saves, settings or rules, and never runs automatically during gameplay. Windows 10/11 and NTFS are required for compaction. Savings depend on eligible duplicates; Explorer's folder-size total may still look unchanged.

**Do not edit compacted cache DDS files directly.** Edit the original BodySkin/Futanari pack and use Refresh, then reselect and confirm in BCNG. Cache files can share physical storage after compaction. This tool is not a scanner of every save's references and is not a bulk cache-delete function.
...
**Register ORefit outfit-correction rules** reads only the outfit-correction information in the OBody ORefit JSON at the full path above. If several MO2 mods provide that file, BCNG reads the winning file. It does not import NPC distribution rules or modify the source JSON.
Outfit-name, plugin and FormID exclusions are shared by CBBE 3BA, BHUNP/UNP and UBE/Necoco: an excluded outfit does not activate either breast or nipple correction. Existing explicit force-refit rules can still activate correction, as can another eligible torso item. With SFS, these rules use the final visible outfit; without SFS, they use worn equipment.
...
### INCOMPATIBLE MODS — DO NOT USE TOGETHER

These mods overlap with BCNG's appearance control. Choose one controller for the affected features.

- **OBody NG (OBody Next Generation):** Body-preset distribution and breast/nipple outfit correction overlap with BCNG. Both controllers can reapply their own morphs to the same actor, overwriting or stacking results. BCNG's OBody/OClothe key cleanup does not stop OBody from running or distributing again.
- **Racial Skin Variance - SPID (RSV)**: Incompatible with BCNG skin control. RSV can reapply its face textures after a character's 3D is rebuilt, overwriting the BCNG face while the body keeps the selected skin. This can cause a darker face, a visible neck seam, or a body/face mismatch during preview or after a later refresh. Do not use RSV and BCNG to control skins together.
- **RaceMenu Selector of Skins - Unique Player Character**: Incompatible with BCNG skin control. Selector reapplies its player Skin Armor and face textures on player 3D load or settings reapplication. This can overwrite a BCNG selection or leave a subsequent skin switch appearing unchanged. Do not use Selector and BCNG to control skins together.

**BCNG does not suspend RSV or Selector, and includes neither built-in conflict prevention nor a separate compatibility patch for them.** A successful preview or double-click confirmation does not make these combinations compatible: a later reapplication can bring the conflict back. Default Skin is a restoration action, not a compatibility workaround.
**[ORefit JSON Master List](https://www.nexusmods.com/skyrimspecialedition/mods/105052) can be used with BCNG.** Install its outfit-correction JSON, then click **Register ORefit outfit-correction rules** in BCNG.

**RaceMenu itself is still required**; the skin Selector above is a separate mod.

### Other compatibility notes and troubleshooting

- **Ordinary texture replacers:** a BCNG skin replaces supported channels while selected; Default Skin restores the underlying skin. This is not simultaneous blending of two complete skin packs or compatibility with another active skin controller.
