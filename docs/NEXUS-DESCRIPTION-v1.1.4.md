...
- **Verified final skin state** — RaceMenu call acceptance alone is not counted as success; BCNG verifies the affected actor's final loaded multipart skin once and performs at most one exact repair without catalog rescans or continuous polling
- **Safe cross-slot limb discovery** — hidden equipment and multi-slot NIFs may anchor exposed hands or feet under another Biped entry, so BCNG scans that actor's fixed Biped set and accepts only exact hand/foot evidence while rejecting body, glove, boot, and genital geometry
