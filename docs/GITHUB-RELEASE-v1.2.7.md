...
- 검증: Release 빌드와 회귀 테스트 27개 통과. 현재 제품 초기화 함수, 취소 경계, 오래된 결과 폐기·메모리 수명, 게시/초기화 경합 400회, 미적용 선택 코세이브 왕복 검사를 포함합니다. 제보된 새 게임 배포 팝업 CTD는 인게임 재현·해결 확인을 하지 못했으며 원거리 NPC 선택·접근 적용도 인게임 확인이 필요합니다. 코세이브 형식과 지원 런타임은 변경하지 않았습니다.
