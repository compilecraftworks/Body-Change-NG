...
{
    // Windows supplies edges, never positions. Only the render thread writes
    ...
    class MouseInputQueue final
    {
        ...
        bool release_{};
    public:
        void Button(int button, bool down) { Push({button, down, 0}); }
        ...
        {
            std::scoped_lock lock(lock_);
            if (events_.size() >= 4096) { events_.clear(); release_ = true; }
