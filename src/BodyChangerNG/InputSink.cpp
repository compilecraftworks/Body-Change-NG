...
namespace bcn
{
    ...
    void InputSink::Register()
    {
        ...
        if (auto* manager = RE::BSInputDeviceManager::GetSingleton()) {
            ...
            SKSE::log::info("Body Changer NG priority keyboard input sink registered");
        } else {
            registered_ = false;
            SKSE::log::warn("Body Changer NG will retry input registration after input is initialized");
        }
    }
    ...
    void InputSink::RequestToggle(const std::string_view source)
    {
        ...
        if (native_ui::Toggle()) {
            SKSE::log::info("Body Changer NG opening shortcut accepted via {}", source);
        } else {
            SKSE::log::warn("Body Changer NG opening shortcut via {} arrived before the native UI was ready", source);
        }
    }
    ...
    void InputSink::StartFocusedWindowHotkeyFallback()
    {
        ...
        try {
            ...
            SKSE::log::info("Body Changer NG focused-window shortcut fallback started");
        } catch (const std::exception& exception) {
            focusedWindowFallbackStarted_ = false;
            SKSE::log::error("Body Changer NG could not start its focused-window shortcut fallback: {}", exception.what());
        }
#endif
    }
