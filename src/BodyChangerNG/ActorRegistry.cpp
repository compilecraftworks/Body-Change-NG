...
namespace
{
    ...
    void SaveState(SKSE::SerializationInterface* output)
    {
        ...
        if (states.size() > kMaxActors) {
            SKSE::log::warn("Body Changer NG actor registry exceeded {}; only the first entries will be saved",
                kMaxActors);
        ...
        if (output && output->OpenRecord(kActorRecord, kRecordVersion)) {
            ...
            if (!ok) SKSE::log::error("Body Changer NG could not write its actor registry cosave record");
        }
        ...
        if (output && output->OpenRecord(kTintRecord, kRecordVersion)) {
            ...
            if (!ok) SKSE::log::error("Body Changer NG could not write its player tint cosave record");
        }
    }
    ...
    void LoadState(SKSE::SerializationInterface* input)
    {
        ...
        while (input && input->GetNextRecordInfo(type, version, length)) {
            if (version != kRecordVersion) {
                SKSE::log::warn("Body Changer NG ignored cosave record {:08X} version {}", type, version);
                continue;
            }
            if (type == kActorRecord) LoadActorRecord(input);
            else if (type == kTintRecord) LoadTintRecord(input);
        }
        SKSE::log::info("Body Changer NG loaded {} actor registry entries from the current save",
...
namespace bcn
{
    ...
    void ActorRegistry::RegisterSerialization()
    {
        ...
        if (!serialization) {
            SKSE::log::error("Body Changer NG could not obtain the SKSE serialization interface");
            return;
        }
        ...
        SKSE::log::info("Body Changer NG registered SKSE cosave callbacks with ID {:08X}", kCosaveID);
    }
    ...
    bool ActorRegistry::NeedsBodyApply(RE::Actor* actor, const std::string_view bodyId,
        const bool useDefault) const
    {
        std::scoped_lock lock(lock_);
        const auto* state = FindValidatedLocked(actor);
        return !state || !state->bodyApplied || state->bodySignature != BodySignature(bodyId, useDefault);
    }
        ...
        const bool useDefault) const
    {
        std::scoped_lock lock(lock_);
        const auto* state = FindValidatedLocked(actor);
        return !state || !state->skinApplied || state->skinSignature != SkinSignature(skinId, useDefault);
    }
