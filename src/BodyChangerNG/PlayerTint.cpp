...
namespace
{
        ...
        const bcn::player_tint::Color color, const std::uint64_t generation)
    {
        ...
        if (path.empty()) {
            SKSE::log::error("Body Changer NG could not cache tint asset '{}'", asset.name);
            return;
        }
        SKSE::log::info(
        ...
        if (applied == 0U) {
            SKSE::log::warn("Body Changer NG could not apply tint '{}': the player has no matching active layer", asset.name);
            return;
        }
        ...
        SKSE::log::info("Body Changer NG applied player tint '{}' to {} base/overlay mask(s)", asset.name, applied);
    }
        ...
        const std::uint64_t generation)
    {
        ...
        SKSE::log::info("Body Changer NG applied tint pack with {} mapped layers", applied);
    }
        ...
        const bcn::PlayerTintBackup backup, const std::uint64_t generation)
    {
        ...
        SKSE::log::info("Body Changer NG restored original player tint layer {}", static_cast<std::uint32_t>(layer));
    }
        ...
        const std::uint64_t generation)
    {
        ...
        SKSE::log::info("Body Changer NG restored {} original player tint layers", restored);
    }
...
namespace bcn::player_tint
{
    ...
    std::vector<Asset> Catalog::ScanDirectory(const std::filesystem::path& root)
    {
        ...
        if (error) SKSE::log::warn("Body Changer NG could not scan player tint masks at {}: {}", bcn::path_text::Utf8(root), error.message());
    ...
    void Catalog::Refresh()
    {
        ...
        SKSE::log::info("Body Changer NG loaded {} player-only tint mask assets", assets_.size());
    }
    ...
    ApplyResult QueueReapplyCurrent()
    {
        ...
        const auto generation = g_tintGeneration.fetch_add(1U, std::memory_order_acq_rel) + 1U;
        SKSE::log::info(
            "Body Changer NG is reapplying tint state after RaceMenu: pack='{}' detail-overrides={} restored-layers={}",
    ...
    ApplyResult QueueRestoreAll()
    {
        ...
        tasks->AddTask([handle, generation] {
            ...
            if (backups.empty()) {
                ...
                SKSE::log::info("Body Changer NG default tint request required no layer restoration");
                return;
            }
