        ...
        if (ImGui::BeginChild("Catalog", ImVec2(0.0F, CatalogListHeight()), true,
            ...
            if (body) {
                ...
                draw->AddText(ImVec2(cursor.x + Scaled(10.0F), cursor.y + Scaled(27.0F)), IM_COL32(160, 181, 193, 255),
                    Text("이 액터의 Body Changer NG·기존 OBody 바디 모프 제거", "Remove Body Changer NG and legacy OBody body morphs from this actor", "移除此角色的 Body Changer NG 与旧版 OBody 身体形态"));
                if (doubleClicked) {
                    FocusCatalogRow(row);
                    confirmRow(row);
                } else if (clicked) {
...
namespace bcn::ui
{
    ...
    void Draw()
    {
        ...
        bool open = true;
        if (!ImGui::Begin("Body Changer NG", &open,
                ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoScrollbar |
        ...
        // latch shared by every Body Changer NG window in this frame.
        ...
        DrawSettingsPopup();
        DrawTintDetailPopup();
        // Tint owns the face framing for the whole tab. Opening or closing the
        // color picker no longer changes the camera; leaving Tint restores the
        // normal left/right presentation immediately.
        bcn::menu_character::Presentation::Get().SetTintDetailFocus(g_activeTab == ActiveTab::tint);
