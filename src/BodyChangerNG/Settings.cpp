...
namespace bcn
{
    ...
    void Settings::Load()
    {
        ...
        if (!std::filesystem::exists(path)) {
            SKSE::log::info("Body Changer NG uses the default opening shortcut {}", data_.openHotkey.DisplayName());
            return;
        }
        ...
        try {
            ...
            if (version != kSchemaVersion) {
                SKSE::log::warn("Body Changer NG ignored unsupported settings schema {}", version);
                return;
            }
            ...
            SKSE::log::info("Body Changer NG loaded opening shortcut {}", data_.openHotkey.DisplayName());
        } catch (const std::exception& exception) {
            data_ = {};
            SKSE::log::error("Body Changer NG could not read {}: {}", path.string(), exception.what());
        }
    }
    ...
    bool Settings::Save() const
    {
        ...
        try {
            ...
            SKSE::log::info("Body Changer NG saved settings to {}", path.string());
            return true;
        } catch (const std::exception& exception) {
            SKSE::log::error("Body Changer NG could not save {}: {}", path.string(), exception.what());
            return false;
        }
    }
