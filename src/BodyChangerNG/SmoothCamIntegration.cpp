...
namespace
{
    ...
    void OnSmoothCamMessage(SKSE::MessagingInterface::Message* message)
    {
        ...
        SKSE::log::info("Body Changer NG connected to SmoothCam V2 camera control");
    }
}
...
namespace bcn::smoothcam
{
    ...
    bool AcquireCameraControl()
    {
        ...
        if (result != ApiResult::ok && result != ApiResult::alreadyGiven) {
            if (!g_denialLogged) {
                SKSE::log::warn("SmoothCam refused Body Changer NG menu camera control ({})", static_cast<std::uint8_t>(result));
                g_denialLogged = true;
            }
            return false;
        }
