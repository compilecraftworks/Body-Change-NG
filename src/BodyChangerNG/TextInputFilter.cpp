...
namespace bcn::text_input
{
    bool Install()
    {
        ...
        if (!layout) {
            SKSE::log::warn("Body Changer NG text-input event filter disabled on unsupported runtime {}",
                version.string("."));
            return false;
        }
        ...
        if (callSite == 0 || std::memcmp(reinterpret_cast<const void*>(callSite),
            ...
            SKSE::log::critical("Body Changer NG input-hook signature validation failed for {}; filter not installed",
                layout->name);
            return false;
        }
        ...
        SKSE::log::info("Body Changer NG text-focus input filter installed for {} ({})",
