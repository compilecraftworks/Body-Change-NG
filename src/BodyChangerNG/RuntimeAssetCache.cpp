...
namespace bcn::runtime_assets
{
        ...
        const std::string_view nameSpace)
    {
        ...
        if (!std::filesystem::is_regular_file(source, error) || error) {
            SKSE::log::error("Body Changer NG texture source is missing: {}", path_text::Utf8(source));
            return {};
        }
        ...
        const auto relative = std::filesystem::path{ "textures" } / "BodyChangerNG" / "Cache" /
            path_text::FromUtf8(nameSpace) / filename;
        const auto destination = std::filesystem::current_path() / "Data" / relative;

        std::filesystem::create_directories(destination.parent_path(), error);
        if (error) {
            SKSE::log::error("Body Changer NG could not create texture cache directory {}: {}",
        ...
        if (!SameFile(destination, source)) {
            ...
            if (error) {
                SKSE::log::error("Body Changer NG could not materialize texture cache {} from {}: {}",
